...
7. **★ 稳定类型化 URI 替代自然语言链接**：`paper:arxiv:…#Thm-1`，且明令禁止中文字符串链接。我们的 `related: []` 是自然语言/双链，**跨文献引用没有稳定 ID**。**（2026-09-17 已决：只采纳到文献级——`citekey` 已有；定理级不做，理由见本文 C5 的落地状态。）**文献库已有 `citekey` 做 ID，但**没有到"定理级实体"的粒度**。
...
- 做什么：① 给 `cards/*.md` 的研读状态加**陈旧提醒**（`unread` 超 N 天、`reading` 超 M 天），并在索引/README 里可见；② 讨论是否把跨文献引用从自然语言升级为**稳定 ID**（`citekey` 已做到文献级；**定理级见下方落地状态：已决不做**）。
- 判据：文献库索引能列出"陈旧卡片"；`check-doc-consistency.mjs` 不漂移。
- **落地状态（2026-09-17）**：① **已做**（索引状态改读卡片真实状态 + 「已停 N 天」/陈旧提醒，逻辑抽到 `scripts/lib/lit-index.mjs` 并配门禁 `test-lit-import.mjs`）；② **已决：不做到定理级**（用户拍板）——`citekey` 已提供文献级唯一 ID，定理靠 `theorems/index.md` + grep 已能定位；引入 `paper:…#Thm-1` 要付出编号与校验的长期成本。理由与重开条件见 `literature/notes/improvement-details-2026-09-17.md` §八第 4 条。
