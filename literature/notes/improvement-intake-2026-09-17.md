...
## 6. 悬置项（2026-09-17 更新：三条已定，只剩一条）

> **本节原先叫「需要你拍板的悬置项」，四条并列。现在三条已由用户拍板并落地，逐条标出结果。** 落地细节见 `improvement-details-2026-09-17.md` 开头的状态总览。

1. **P0-1 的确定性代理量选哪一个** → **✅ 已定**：按我建议，**只用「✅ / ❌」两个显式信号**（`harmed`、`needs_review`、`verified_by`），**不把"用户继续追问"当负分**。实现为 `hook.gain ∈ {−1, 0, +1}`，见 `dsh/preset/math-memory.mjs` 的 `cardGain`。
2. **P1-4 探针升级是否一次性做完** → **✅ 已定：分批**。已做 `Avg-R`（§3）。**未做**：Cost 与效果并看、双场景对照（后者两套用例集不可比，见 `improvement-details` 第 9 项）。
3. **P2 注入预算档位是否值得做** → **✅ 已定：做**。档位 + 设置页入口均已落地（`44ddb47`）；走库内 `.deepseek/config.md`，**没有新增环境变量**（所以 `docs/env-vars.md` 的双向守卫不受影响）。
4. **P1-5 的定理级引用是否要做** → **⏳ 仍未定**：收益是精确引用到某条引理（形如 `paper:…#Thm-1`）；成本是给 `theorems/` 引入新的 ID 契约与生成/校验。**这是本节唯一还开着的项。**
