...
> **状态（2026-09-17 更新）**：本文**成稿时**是评估与提案；**其中绝大多数提案随后已被执行**（用户批准后分三批落地，共 20+ 提交）。**执行结果与逐项状态以 `improvement-details-2026-09-17.md` 开头的「落地状态总览」为准**——本文保留的是**当时的判断依据**（为什么提、来源、优先级），不回头改写。未做清单的唯一权威仍是 `docs/handoff.md` §7。
...
最值得立刻做的三件事（都**零 token**、都不触碰"插件不调模型"红线）——**成稿时的排序；三件后来都做了**：
...
- **实体级稳定引用**（`paper:arxiv:…#Thm-1`）：我们 `citekey` 已做到文献级。**定理级 → 已决：不做**（2026-09-17 用户拍板）；理由见 §6 第 4 条。
...
## 6. 悬置项（2026-09-17 更新：**四条全部落定**）

> **本节原先叫「需要你拍板的悬置项」，四条并列。四条均已由用户逐条拍板，结果如下。** 落地细节见 `improvement-details-2026-09-17.md` 开头的状态总览（那里是唯一权威的逐项状态）。

1. **P0-1 的确定性代理量选哪一个** → **✅ 已定**：只用「✅ / ❌」两个显式信号（`harmed`、`needs_review`、`verified_by`）；**"用户继续追问"被否掉**。实现为 `hook.gain ∈ {−1, 0, +1}`，见 `cardGain`。
2. **P1-4 探针升级是否一次性做完** → **✅ 已定：分批**。已做 `Avg-R`（§3）。**未做**：Cost 与效果并看、双场景对照（后者两套用例集不可比）。
3. **P2 注入预算档位是否值得做** → **✅ 已定：做，且已完成**（档位 + 设置页入口，`44ddb47`）；走库内 `.deepseek/config.md`，**没有新增环境变量**。
4. **P1-5 的定理级引用是否要做** → **✅ 已定：不做**（2026-09-17）。理由与"什么条件下才值得重开"写在 `improvement-details` §八第 4 条：`citekey` 已做到文献级唯一 ID，定理靠标题 + grep 已可定位；引入 `paper:…#Thm-1` 契约要付出编号与校验的长期成本。**除非出现具体的引用歧义案例，否则不要再提。**

## 7. 落地顺序建议（**成稿时的计划；实际执行如下**）

> **实际就是按这个顺序走的**（用户批准后分三批、逐项提交）。括号里是实际结果。

```
第一批（P0，全部零 token、互不依赖，可一次提交一组）
  P0-5 写入原子性 + 模型只提案 + 增量回执   ← 先立契约（它是其余各项的前提）
       实际：所有权表 + 脏值免疫已做；增量回执符号延后（理由见 details 第 5 项）
  P0-4 状态机"未达状态即禁止"              实际：已做
  P0-3 去重排序 + 枢纽保护                 实际：已做
  P0-2 依赖方向 + 级联复审                 实际：已做
  P0-1 增益信号                            实际：已做（gain，负值压到未评级卡之下）

第二批（P1，需要小设计）
  P1-1 跨 episode 独立支撑 → P1-2 三道结晶门
       实际：独立支撑只交数据层，接进 promote 被否决；结晶门只做接地门
  P1-3 收窄边界 + 反模式结构化             实际：已做（收窄挂 inapplicable；decision_guidance）
  P1-4 探针升级                            实际：只做 Avg-R（分批）；双场景对照未做
  P1-5 文献库生命周期 + 来源登记           实际：陈旧提醒 + 索引状态已做；来源登记未做

第三批（P2，动代码/动检索核心，与检索 v4 一起评）
  P2 注入预算档位 / 三层级联与回退
       实际：档位（含设置页入口）已做；三层级联只补协议文本、检索核心未动
```
...
- **本轮（成稿时）**：**未运行任何 live/烧钱验收**（AGENTS.md §4 要求先问用户）；**未改动任何代码**；全部动作都是读、导入文献、写研读记录。
- **后续（同日，用户批准后）**：提案已分三批执行——共 20+ 提交，`test-memory` 240 → 297，新增门禁 1 个（总数 39 → 40）。**执行结果以 `improvement-details-2026-09-17.md` 的落地状态总览为准**；仍未做的子项列在那里（第 5/7/9/10 项的部分子项 + 第 6 项被否决的部分），并已同步到 `docs/handoff.md` §7。
