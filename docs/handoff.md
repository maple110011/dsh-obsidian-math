...
| `scripts/test-memory.mjs` | 零 token 记忆回归（297 项断言，进 `npm test`） |
...
```bash
npm test                        # 297 项零 token 回归 + 47 项路由回归 + 8 项认证 + 32 项反代 + 安装器 e2e + 漂移 + 五守卫 + 文档一致性 + 中英配对 + 语法检查（= node scripts/run-gates.mjs）
...
| **预算档位对注入体积的实际影响** | 已量：真实库三档注入 **7864 / 8250 / 8890** 字符（硬上限 18000），**三档几乎无差别**——除 `episodes/index.md`（2647 字符 / 预算 1200，保尾部）外所有文件都没超预算。**结论：标准档目前偏保守但无影响**；库变大或时间线变长后档位才有实际差别 | 观察，无需动作 |
| **审计里每张 hook 卡多读一遍文件** | `writeGain(true)` 使审计对每张 hook 卡多一次 `readFileSync`（此前 2 次、现 3 次）。每日一次、卡片百级，非热路径。修法是复用 `cardEntries` 已读的文本 | 低 |
| **陷阱 79–81 是新增的** | handoff §4 的坑现在到 **81**；AGENTS.md §1 与 §6 里"69 条历史陷阱"的字样已过期，应改为**按实际条数**描述（本轮未改，以免与守卫锚点冲突） | 低 |
