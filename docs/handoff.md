...
| `scripts/test-memory.mjs` | 零 token 记忆回归（321 项断言，进 `npm test`） |
...
```bash
npm test                        # 321 项零 token 回归 + 47 项路由回归 + 8 项认证 + 32 项反代 + 安装器 e2e + 漂移 + 五守卫 + 文档一致性 + 中英配对 + 语法检查（= node scripts/run-gates.mjs）
