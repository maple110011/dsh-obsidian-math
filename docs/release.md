...
## 5. 版本更新说明（CHANGELOG 段落）的写法

`CHANGELOG.md` 的版本段落**会原样成为 Release 正文**（`release.yml` 抽取），所以它同时是**面向用户的更新说明**。写法规则（2026-09-18 用户明确要求）：

- **正文开头只写一段「本版主要包含哪几个方面」的摘要**——3–4 个方面，每个方面点出关键机制即可，让读者十秒内知道这版在做什么。
- **不要在段落里写内部过程**：累计了多少个提交、哪个版本漏发过、哪些内容曾长期躺在 `[Unreleased]`、门禁/断言数从多少涨到多少（后者属 `Internal` 小节，可保留但不必进摘要）。这些是**维护者细账**，去处是 `docs/changelog.md`。
- 摘要之外的小节（`Added` / `Changed` / `Fixed` 等）照旧按用户价值写。

**为什么写进手册**：这是第二次出现"把维护者视角写进用户说明"的返工（第一次是 0.7.8 的摘要，用户读后指出不需要"本版是 X 之后累积的全部改动、共 N 个提交"这类交代）。规则写在手册里，比留在某次对话里更可能被下一次发版的人遵守。
