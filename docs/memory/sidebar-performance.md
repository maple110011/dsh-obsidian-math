...
- ⇒ **Obsidian 侧栏面板的交互成本（面板展开/收起、iframe 重排/合成、宿主渲染）从未被测量**：探针跑在独立 Chromium 里，**看不到也点不到** Obsidian 的 Electron 界面。这一条是本次调查真正的空白，见 §9 第 5 项。
...
- **`Obsidian 侧栏面板` 这一侧的交互成本（真正的空白）**：本节前面所有数字都来自**独立浏览器里的 dsh 侧栏**。要量 Obsidian 侧，必须驱动 **Obsidian 自己**（它也是 Electron）：
  1. 以远程调试端口启动 Obsidian（`obsidian.exe --remote-debugging-port=9222`，需先完全退出正在运行的实例），Node 用内置 `WebSocket` 连它——**同一套 CDP 代码**，换 target 即可；
  2. 用它点真正的**面板展开/收起**（Obsidian 的 ribbon / 命令，而不是 dsh 自己的按钮），并读 `Performance.getMetrics()`；
  3. 读 iframe 内部需要连到那个 frame 的 target（Electron 开了 CDP 就能拿到跨源 frame），这样 §6 的 DOM 规模也一并量到。
  **为什么必须这样**：headless Chromium 里没有 Obsidian 的宿主渲染与面板重排，探针再准也测不到这一层。
...
5. **`Obsidian 侧栏面板` 这一侧从未被测过（本次调查真正的空白）**：所有 CDP 数字都来自**独立浏览器里的 dsh 侧栏**（§0.1），而用户报告的动作是「在 Obsidian 里展开侧栏」。探针跑在 headless Chromium，**看不到也点不到 Obsidian 的 Electron 界面**，所以「面板展开/收起 + iframe 重排/合成 + 宿主渲染」这部分只有推理没有测量——原因 3/4 的处置属于此类。**下一步用 §6 的新办法（给 Obsidian 开 `--remote-debugging-port`，同一套 CDP 代码换 target）把这一层量出来**；在量到之前，不要声称「侧栏卡顿已定位」覆盖了 Obsidian 侧。
