...
### 0.3 「刚启动不卡、用久了才卡」：卡顿随当前会话的渲染树规模增长（2026-09-14）

用户的新观察：**Obsidian 刚启动时不卡，卡顿是后来才出现的**；关掉其他程序 + 重启 Obsidian 后连续 2 小时不卡。

**实测（同轮同一 harness，`scripts/qa/sidebar-newchat-probe.mjs`，headless Chromium + 真实 dsh + 真实皮肤 + 经插件反代）**：

| 文档元素数 | 每次样式重算 | 帧 >50ms | 最差帧 | LoAF |
|---|---|---|---|---|
| 730（空会话 hero） | **1.2 ms/op** | 0–1 | 50–67 ms | 1–3 |
| 12 731（往会话里注入 3000 行占位内容） | **19.9 ms/op（16.6×）** | **52** | **533 ms** | 54 |

对照 §0.2 在**真实 Obsidian** 里的读数（13 102 元素、410 次重算共 22.76 s = **55 ms/op**）：同一量级，而且真实的会话节点比注入的纯 div 更贵。⇒ **§0 原因 11（DOM 规模）不是"另一个可能"，它就是"用久了才卡"的直接解释**：

1. 面板里当前会话越长，渲染树越大，侧栏开合（CSS grid 轨道切换）每次要重算的样式面越多；
2. 这与"启动时"无关——启动时页面在新会话（几百个元素）；
3. **关掉其他程序不会让这份文档变小**，所以"当时没有马上恢复"；恢复发生在**重启 Obsidian**（iframe 重建、回到新会话）。用户"关程序 + 重启后 2 小时不卡"里的有效动作是**重启**，不是关程序。

内存压力（本机 14 GB，关掉一堆程序后空出约 1 GB）是**放大器**而不是主因：它让渲染进程更容易被换出/触发 GC，但即使内存充足，13k 节点的重算成本也依然是 55 ms/次。

**对用户可用的缓解（当前唯一）**：把超长会话收尾/重开，别让一个会话永久滚动下去；必要时重载面板。**上游可推动的**：dsh 前端让侧栏宽度别再走整树重排（§0.2 已给出可复现数字）。

**⚠️ 自我更正（同日晚些时候）**：上表里 12 731 那一行是**往页面注入 3000 行 div** 造出来的，而 `data-chat-flow` 并不总是挂在可注入的位置（控制台打出 `flow=DIV[other]`，注入实际落在 `body`）。所以它只证明「**同一页面上，文档越大、重算越贵**」这条缩放关系，**不能**用来预测「你的会话写到 13k 节点时一定会卡」。真实数字只认 §0.2 在真实 Obsidian 上的读数（13 102 节点、55 ms/次）。要复测请打真实实例：`node scripts/qa/sidebar-newchat-probe.mjs --entry=http://127.0.0.1:3180/`。另外，「点了新建会话没反应」与本节**无关**——那是 preset 挂载失败（见 `handoff.md` 坑 70）。
...
# 「新建会话」为什么点不动 + 卡顿随文档规模增长（2026-09-14 新增；见 §0.3）
node scripts/qa/sidebar-newchat-probe.mjs --vault="D:\Obsidian笔记数据库" --bloat=3000
```
...
4. ✅ **iframe 内 DOM 规模与长会话的影响已于 2026-09-14 量化**（§0.3）：730 元素 → 1.2 ms/重算；12 731 元素 → 19.9 ms/重算（16.6×）、帧 >50ms 52 个、最差帧 533 ms。真实 Obsidian 在 13 102 元素时是 55 ms/重算。**这是"用久了才卡"的直接解释**；尚未量到的是"多久会涨到这个规模"（取决于会话长度与工具输出体量），以及 3080 那一侧的长会话。
