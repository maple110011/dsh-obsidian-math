...
**★ 一条我试了、发现不成立、于是如实登记而没有硬改断言的事**：「档位真的改变注入体积」这条**在夹具下不成立**——`compact` 与 `rich` 得到**完全相同**的 2120 字符，而 profile 文件是 6805 字符、渲染出来的 profile 段却只有约 354 字符。`clip` 就是普通切片，`buildMemorySection` 里也没有 profile 的二次处理，所以中间有什么在缩短它，**我连猜两次都没对**。
> 按「以不懂装懂为耻」，我没有继续调夹具直到断言变绿——**一个被调成会通过的测试什么也证明不了**。改为在测试里写明这个异常与待办（"先查清是什么在约束 profile 段，再断言档位效应"），并把断言降级为"档位已穿进 builder 且不抛错"。**这是一个真实待办，不是已修。**
