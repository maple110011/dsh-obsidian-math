...
> **本版是 0.7.7 之后累积的全部用户可见改动**（43 个提交）。0.7.7 发布时 `[Unreleased]` 是空的，其后的「文献研读 → 治理闭环」两批与「WikiSkill 研读 → 记录纪律」一批**都没进过任何 Release**——所以这些能力是你升级到 0.7.8 才会第一次拿到的。主题：**把「写入之后怎么知道它有没有用」这件事做成确定性的治理闭环**，并把三条只写在文档里、无法检查的纪律变成可判定的检查。
...
——体检每天都会从同样的文件推出同样的建议，此前"这张卡上周报过、当时决定先不动"无处记录，于是同一张卡会被当成新发现反复报。现在每次体检把建议追加进 `.deepseek/cache/audit-ledger.jsonl`（**只由插件写、只增不改**），并在报告里**把"本次新动作"与"此前已在账、今日仍成立"分开**：历史条目只带首次出现日期与次数，**不要求你做任何动作**。判定仍归你与助手，台账只记录。规格见 `docs/memory/design.md` §8.1。
- **记忆：卡片有了尺寸上限，超限会被点名**——正文超过 **20 行**、策略卡 move 超过 **5 个**时，体检会单独报一条并说明该怎么办（拆成原子卡 / 把过程挪进事件卡 / 按触发条件排序并移出不触发的 move），**不会替你改**。上限来自实测分布（种子库正文 7–8 行、move ≤ 3），刻意宽于正常写作。
- **记忆：破坏性写入不再可能"无声失败"**——自动归档以前把每个错误都吞掉，重命名或索引写入失败时报告照样说"已归档 N 张"。现在索引改写走**临时文件 + 读回校验 + `.bak` 备份 + 原子替换**（任一步失败，原文件一字不动），失败会作为"降级"写进体检报告，并说明卡片是否已经移动、可据 `.bak` 修复。
- **记忆：注入内容被预算截断时会自报**——此前超预算的段落只以结尾一个 `…` 表示，而"原文本来就这样"与"被截了"看起来一模一样，于是模型可能把残片当完整证据。现在截断处会写明**原文总长度**（`……［截断：全文 N 字符］`），标记不占预算、不改变各层实际注入的内容量。
- **记忆：索引行的说明不再可以空着**——`records/` / `templates/` / `strategy/` 的索引里那句"一句话"决定读者要不要打开卡片，而此前 `- [[x]]` 或 `- [[x|?]]` 也能通过体检。现在体检会确定性检查**描述是否为空白或过短**并点名（写明该补什么：是什么 + 为什么有效 + 怎么做），**只报告、不代写**；行首不是 `[[链接` 的行会作为「索引行不合契约」单独报（这种卡按名字找不到）。
...
- **门禁 39 → 40：文献库索引逻辑** `scripts/test-lit-import.mjs`（10 项，**进程内**跑，不启子进程）。
- **记忆：注入预算档位**——设置页新增「注入预算」下拉框（`compact` / `standard` / `rich`），控制每次开机注入的**导航层**大小（主题/记录/模板/事件索引）。**它不限制正文**：答案内容仍由 `note_recall` 按需检索。**默认 `standard` 与改动前的数值逐字相同**，所以不选就什么都没变。档位写在 `.deepseek/config.md` 的 `budget`；显式写在 preset 配置里的 `budget` 优先级更高。详见 `docs/changelog.md`。
- **记忆：卡片新增机器维护的 `hook.gain`**（−1 / 0 / +1）。`uses` 数的是"被检索出来几次"，不等于"帮上忙"；`gain` 只由你显式点的 ✅（+1）/ ❌（−1）推出，无反馈则不写这一行（中性）。**负值会把该卡排到未评级卡之后**——这是此前 `harmed` 做不到的（它只进体检报告、不影响排序）。
- **记忆：拒绝即收窄适用范围**——你点「本次场景不适用」时，插件会把本次检索的上下文特征追加进该卡的 `not_applicable_when`，于是**下次同类查询会直接排除这张卡并说明原因**，而不是只把成功率降一点。注意「❌ 这张卡有错」**不会**改边界（那是内容判定，不是适用范围判定）。
- **记忆：策略卡分出「候选」与「可依据」**——`note_strategy` 现在把 `status: candidate` 的卡单独列出并标明"只能当线索、不得当作已验证技巧引用"。此前两类混在一起、输出里连状态都没有，模型无从分辨。
- **记忆：策略卡新增 `decision_guidance`（建议 / 避免，成对、各带证据）**，`note_strategy` 会渲染成「✔ 建议 / ✘ 避免」。只写"该怎么做"记不住教训，成对写才能把一次失败固化成可迁移的判断。
- **记忆：卡片新增可选 `depends_on`（有方向的依据链）**——与无方向的 `related` 不同，它让"某张卡的依据变了之后，哪些卡需要重读"变成体检可以查出来的事（体检新增「下游待复查」段）。同时新增「结构枢纽」段（被多张卡引用的卡单列，提醒改动影响面）。
- **文献库：索引状态改为显示卡片真实状态**。此前索引的「状态」列取的是首次建卡时的机器默认值（恒为「未读」），而卡片本身跨导入是保留的——于是全库卡片其实都已蒸馏、索引却全显示「未读」。现在改读卡片；并在卡片 `研读状态` 写 `- 状态更新：YYYY-MM-DD` 时给出「已停 N 天」与陈旧提醒（未读/待提炼 ≥ 7 天、研读中 ≥ 30 天）。
- **QA：引擎探针新增 §3「检索成本 Avg-R」**（零 token）——`Avg-R = Σ(未找到 ? 10 : 目标排名) / 用例数`，并报中位数、最差名次、名次分布。命中率把"第一个就对"和"翻到第 7 个才找到"记成同一个分数，这个数字把差别量出来。只报告、不做断言，用途是改检索前后的可比基线。
...
- **侧栏卡顿：补上了宿主侧（Obsidian）的实测，结论是"不花在宿主上"**。此前所有性能数字都来自**独立浏览器里的 dsh 侧栏**，而用户报告的动作是「在 Obsidian 里展开侧栏」——那一层从未测过。现在用新增的 `scripts/qa/sidebar-attach-probe.mjs`（给 Obsidian 开 `--remote-debugging-port`，附着到宿主与侧栏 iframe **两个 target 并发**采样，**只读**观察用户手动点击）测出：同一 90 秒内**宿主 0 帧 >33ms、最差 18 ms、LoAF 0、`RecalcStyle` 0.067 s**；**iframe 63 帧 >33ms、最差 2183 ms、`RecalcStyle` 22.76 s（410 次，55 ms/次）**，且掉帧秒精确对齐三次侧栏开合。⇒ 卡顿全在 dsh 面板侧，主因是**布局/样式重算**（文档 13 102 个元素），不是脚本空转；面板收起/展开这一侧（iframe 隔离、隐藏即停）属廉价保险。剩余一项 A/B（关掉「侧栏加载皮肤动态装饰」复测以分离皮肤脚本的贡献）登记在 `handoff.md` §7。
...
- **`scripts/qa/sidebar-attach-probe.mjs`（按需运行，不进门禁）：侧栏性能的"附着模式"**。既有 `sidebar-perf-probe.mjs` 自己起 dsh + 自己起 headless Chromium，只能测**独立浏览器页**里的 dsh 侧栏；新脚本附着到**你正在用的 Obsidian**（Chromium 调试端口），因此能测**宿主侧**——面板开合、iframe 重排/合成、Obsidian 自身的渲染成本。两个 target（宿主 `app://obsidian.md` + iframe `127.0.0.1:3180`）**并发**记录同一段墙钟，用只读 observer 给每次开合打时间戳；**不派发任何输入事件**。用法与三条测量纪律（窗口被遮挡时 rAF 不触发、两侧必须并发、ResizeObserver 首次回调不是用户操作）写在文件头。
...
- **`check-client-bundle.mjs` 在 Windows 上报"产物过期"，其实是换行**：该守卫把提交进仓库的 `dsh/client-panel/lib/client.js` 与一次全新构建**逐字节**比较，却没归一化换行。Windows runner（`core.autocrlf=true`）检出的产物是 CRLF（14456 字节），构建输出是 LF（14445 字节），差值正好是文件的 **11 个换行** ⇒ **同一个 commit** 在 `test (windows-latest)` 上红、在 `test (ubuntu-latest)` 上绿。这是 `handoff.md` 坑 57 的复发（main.js 的新鲜度守卫早就用 `\r\n → \n` 修过同一件事），现已对齐。**变异验证**：强制 CRLF 检出 → 修前输出与 CI **逐字一致**（`committed 14456 bytes vs fresh 14445 bytes`）、修后 OK；再故意让产物过期 → 仍然 FAIL。**顺带记下它为什么本地一直显不出来**：本机沙箱禁止 esbuild 子进程，守卫如实 SKIP、只跑形状检查后 exit 0，而汇总是按退出码统计的，于是连续几轮"34/34 全绿"里这一条其实什么都没比（坑 69；`AGENTS.md` §4 已就地更正那句断言）。
