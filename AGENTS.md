...
| 上手/交接：改哪里、跑什么、**82 条历史陷阱** | [`docs/handoff.md`](docs/handoff.md)（§2 入口地图、§4 陷阱、§7 未做清单） |
...
- 完整清单见 `docs/handoff.md` §4（**82 条**，数字由 `docs/handoff.md` §4 顶部的标记行 + `node scripts/check-doc-counts.mjs` 守卫保证不腐烂）——改动前扫一遍与你要动的东西相关的条目。
