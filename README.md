...
- **Unified recall (retrieval v3)**: `note_recall` ranks the WHOLE vault — user notes AND memory layers (cards with `hook:` frontmatter weighting, memos, topics, theorem/episode indexes) — with BM25 + CJK char containment + success-rate prior in one call; kind-aware passages follow LeanSearch v2's structured-passage lesson. The protocol then has the agent read and judge the top 2-3 hits in full (empty result = reformulate once, then admit nothing found).
...
The `obsidian` profile is fail-closed by design: writes are confined to the vault (`workspace-write`), interactive permission-escalation prompts are **disabled** (`approval: never`, so an accidental click cannot widen the boundary), and the toolset contains no delete/rm tools. Setting `DSH_PERMISSION_MODE=danger-full-access` explicitly (and restarting the service) re-enables the interactive escalation prompts (`approval: ask`); the sandbox itself stays `workspace-write`, so every widening beyond the vault still requires an explicit per-operation confirmation.
